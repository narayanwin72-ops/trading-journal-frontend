...
export default function SwingTradeForm() {
  ...
  /* ===== MERGED EQUITY SYMBOLS ===== */
  const equitySymbols = getTradeSymbols("EQUITY");

  /* ===== SEARCH STATE (SAME AS OTHER FORMS) ===== */
  ...
  useEffect(() => {
    if (editTradeId) {
      ...
      if (trade && trade.tradeType === "SWING") {
        setForm({ ...emptyForm, ...trade });
        setSymbolSearch(trade.stock || "");
      }
    }
  }, [editTradeId, trades]);
  ...
  function getBrokerCapital(broker) {
    ...
    transactions.forEach((t) => {
      if (t.broker === broker) capital += Number(t.amount);
    });
    return capital;
  }
  ...
  return (
    <div style={container}>
      <h3 style={{ marginBottom: "16px" }}>
        {editTradeId ? "Edit Swing Trade" : "Swing Trading Entry"}
      </h3>
      ...
      <Section title="Trade Dates">
        <Grid>
          ...
          {/* ===== STOCK / SYMBOL (SEARCHABLE – EQUITY) ===== */}
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              onBlur={() => {
                setTimeout(() => {
                  if (!equitySymbols.includes(symbolSearch)) {
                    setSymbolSearch("");
                    update("stock", "");
                  }
      ...
      <Section title="Strategy & Psychology">
        <Grid>
          ...
          <Select
            label="Timeframe"
            options={["15 min","30 min","45 min","1 H","4 H","1 Day","1 Week","1 Month","Multi"]}
      ...
      <button style={saveBtn} onClick={handleSave}>
        {editTradeId ? "Update Swing Trade" : "Save Swing Trade"}
      </button>
    </div>
  );
}
